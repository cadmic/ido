...
static int func_00474DC0(struct IChain *ichain) {
    ...
    switch (ichain->type) {
        case isconst:
            return ichain->isconst.number.intval;
        ...
        case isop:
            if (optab[ichain->isop.opc].is_binary_op) {
                left = func_00474DC0(ichain->isop.op1);
                right = func_00474DC0(ichain->isop.op2);
...
*/
static void strength_reduce(struct Statement *stat, struct Graphnode *node) {
    int increment; // sp80
    bool overflow; // sp7B
                        ...
                        if (inpath && (ichain->type != isop || (ichain->isop.opc != Uilod && ichain->isop.opc != Uirld))) {
                            ...
                            if (factor != 0 || multiplier != NULL) {
                                ...
                                *srcand = (struct StrengthReductionCand){0};
                                ...
                                if (multiplier != NULL) {
                                    ...
                                    if (ichain->isop.srcand != nota_candof && !node->in_rolled_preloop) {
                                        if (ichain->isop.srcand != NULL ||
                                            ...
                                            ichain->isop.stat = NULL; // maybe temploc?
                                        }
                                    }
                                } else {
                                ...
                                // unused
                                if (mipsflag == 3 && inloop && looptab[loopno].unk9) {
